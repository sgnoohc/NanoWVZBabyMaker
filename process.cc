...
//********************************************************************************
    ...
    options.add_options()
        ...
        ("y,year"        , "Year of the analysis"                                                                                , cxxopts::value<int>()->default_value("2018"))
        ("j,nsplit_jobs" , "Enable splitting jobs by N blocks (--job_index must be set)"                                         , cxxopts::value<int>())
        ("I,job_index"   , "job_index of split jobs (--nsplit_jobs must be set. index starts from 0. i.e. 0, 1, 2, 3, etc...)"   , cxxopts::value<int>())
        ("d,debug"       , "Run debug job. i.e. overrides output option to 'debug.root' and 'recreate's the file.")
        ("D,is_data"     , "Boolean flag of whether it is a data sample or not")
        ("h,help"        , "Print help")
        ;
    ...
    ana.processor = new RooUtil::Processor(ana.tx);
    ...
    // TODO: Set the year properly (someone might implement it)
    //_______________________________________________________________________________
    gconf.year = result["year"].as<int>();
...
//********************************************************************************
    ...
    ana.cutflow.addCut("Weight", [&]() { return 1; }, [&]() { return ana.tx->getBranch<float>("genWeight");} );
    ana.cutflow.addCutToLastActiveCut("NAnyLepGeq3", [&]() { return ana.tx->getBranchLazy<vector<float>>("lep_pt").size() >= 3; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("NVetoLepGeq3", [&]() { return ana.tx->getBranch<int>("nVlep") >= 3; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("NVetoLepGeq4", [&]() { return ana.tx->getBranch<int>("nVlep") >= 4; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("BVeto", [&]() { return ana.tx->getBranch<int>("nb") == 0; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("FindZLepCands", [&]() { return ana.tx->getBranch<int>("lep_ZCand_idx1") >= 0; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("FindWLepCands", [&]() { return ana.tx->getBranch<int>("lep_Nom_idx2") >= 0; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("ChannelEMu", [&]() { return ana.tx->getBranch<vector<int>>("lep_id")[ana.tx->getBranch<int>("lep_Nom_idx1")] * ana.tx->getBranch<vector<int>>("lep_id")[ana.tx->getBranch<int>("lep_Nom_idx2")] == -143; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("HighMT", [&]() { return ana.tx->getBranch<float>("lep3MT") > 40. and ana.tx->getBranch<float>("lep4MT") > 20.; }, UNITY);
    ...
    // Book modules to the central processor
    ana.processor->AddModule(new wvzModule::METModule());
    ana.processor->AddModule(new wvzModule::LeptonModule());
    ana.processor->AddModule(new wvzModule::JetModule()); // Must come after lepton for proper overlap removal
    ana.processor->AddModule(new wvzModule::AnalysisVariableModule());
    ana.processor->AddModule(new wvzModule::BDTInputVariableModule());

    // Create output branches
    ana.processor->AddOutputs();
    ...
    while (ana.looper.nextEvent())
    {
        ...
        ana.processor->SetOutputs();

        //To save use the following function
        ana.cutflow.fill();

        if (ana.cutflow.getCut("NVetoLepGeq3").pass)
        {
            ana.processor->FillTree();
        }

    }

    // Writing histogram outputs file
    ana.cutflow.saveOutput();

    // Writing ttree outputs to file
